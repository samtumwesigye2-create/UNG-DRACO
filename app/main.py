...
def operator_video() -> FileResponse:
    return FileResponse(STATIC_DIR / "draco_operator.html")
...
def process_observation(observation_id: str, db: Session = Depends(get_db), principal: Principal = Depends(require_roles("draco_analyst", "draco_admin"))) -> dict:
    ...
    try:
        related=correlate_observations(db,observation.id); track=upsert_track(db,observation,related); product=build_assessment(db,track); alerts=evaluate_watches(db,track)
        append_audit_event(db,actor_id=principal.subject,action="track.processed",resource_type="track",resource_id=str(track.id),correlation_id=correlation_id,result="success",request_metadata={"observation_id":str(observation.id),"related_observation_ids":[str(i.id) for i in related]})
...
def list_sources(db: Session=Depends(get_db),principal: Principal=Depends(require_roles("draco_source_admin","draco_admin"))) -> list[dict]:
    return [{"id":str(i.id),"real_name":i.real_name,"contact":i.contact,"notes":i.notes} for i in db.scalars(select(Source).order_by(Source.created_at.asc())).all()]
...
def list_watches(db: Session=Depends(get_db),principal: Principal=Depends(require_roles("draco_analyst","draco_admin"))) -> list[dict]:
    ...
    return [{"id":str(i.id),"target":i.target,"target_type":i.target_type,"active":i.active,"start_date":i.start_date.isoformat() if i.start_date else None,"end_date":i.end_date.isoformat() if i.end_date else None} for i in rows]
